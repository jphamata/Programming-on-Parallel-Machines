...
void Node0()
{
    ...
    for (I = 1; I <= N / 2; I++) {
        ...
        if (ToCheck % 3 > 0) Error = MPI_Send(&ToCheck, 1, MPI_INT, 1, PIPE_MSG, MPI_COMM_WORLD);
    }
...
void Node1()
{
    ...
    while (1) {
        ...
        Error = MPI_Recv(&ToCheck, 1, MPI_INT, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &Status);

        if (Status.MPI_TAG == END_MSG) break;

        if (ToCheck % 5 > 0) Error = MPI_Send(&ToCheck, 1, MPI_INT, 2, PIPE_MSG, MPI_COMM_WORLD);
    }
...
void Node2()
{
    int Error, /* not checked in this example */
        ...
        IsComposite,
        PrimeCount,
        ToCheck; /* current number to check from Node 0 */
    MPI_Status Status; /* see below */
    ...
    while (1) {
        Error = MPI_Recv(&ToCheck, 1, MPI_INT, 1, MPI_ANY_TAG, MPI_COMM_WORLD, &Status);
